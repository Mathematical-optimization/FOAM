...
class ShampooMonitor:
    ...
    def save_heatmap_plots(self):
        ...
        def sort_key_wrapper(x):
            parts = x.split('.')
            try: param_idx = int(parts[0])
            except ValueError: param_idx = parts[0]
            block_name = parts[1] if len(parts) > 1 else ""
            block_num = -1
            if block_name and block_name[-1].isdigit():
                num_str = ''
                for char in reversed(block_name):
                    if char.isdigit(): num_str = char + num_str
                    else: break
                if num_str: block_num = int(num_str)
        ...
        def generate_and_save(dim_idx, dim_name):
            ...
            for bid in block_ids:
                ...
                row_data = []
                has_data = False
                if dim_idx not in self.block_history[bid]: continue
                for e in epochs:
...
def patch_shampoo_optimizer(optimizer, monitor, current_epoch_fn, eigh_monitor):
    ...
    def patched_compute_single_root_inverse(
        ...
        try:
            ...
            if len(parts) >= 2: block_id = f"{parts[0]}.{parts[1]}"
            else: block_id = parts[0]
            d_idx = int(parts[-1])
...
def validate_on_trainset(model, train_loader, criterion, device, rank, world_size):
    """Full-batch training loss 계산 (with torch.no_grad & model.eval)"""
    model.eval()
    running_loss = 0.0
    num_batches = 0
...
def train(args: argparse.Namespace):
    ...
    # (A) Regular Training Dataset (with Augmentation)
    ...
    # [MODIFICATION] (B) Clean Training Dataset (No Augmentation, for Full-batch Loss Calculation)
    # .with_transform() creates a new view of the dataset with different transforms without affecting the original
    clean_train_dataset = dataset['train'].with_transform(functools.partial(apply_transforms, transform=val_transform))
    
    # Sampler for clean dataset (shuffle=False is sufficient for evaluation)
    clean_train_sampler = DistributedSampler(clean_train_dataset, num_replicas=world_size, rank=global_rank, shuffle=False)
    clean_train_loader = DataLoader(
        clean_train_dataset, 
        batch_size=args.batch_size, 
        sampler=clean_train_sampler,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # Simple logging for steps (optional to reduce communication overhead)
        ...
        # [MODIFICATION] Calculate Clean Full-batch Training Loss for Epochs 81~90
        # Epoch is 0-indexed, so 80 corresponds to the 81st epoch.
        clean_full_train_loss = None
        if epoch >= 80:
            if global_rank == 0:
                print(f"Calculating Clean Full-batch Training Loss for Epoch {epoch+1} (No Augmentation)...")
            
            # Use clean_train_loader (No Augmentation)
            # validate_on_trainset uses model.eval() and torch.no_grad() internally
            clean_full_train_loss = validate_on_trainset(
                model, clean_train_loader, criterion, local_rank, global_rank, world_size
            )
        ...
        if global_rank == 0:
            print_msg = (f"Epoch {epoch+1}: "
                         f"Val Acc {val_acc:.2f}%, Val Loss {avg_val_loss:.4f}, Time {epoch_duration:.2f}s")
            
            if clean_full_train_loss is not None:
                print_msg += f", Clean Train Loss {clean_full_train_loss:.4f}"
            ...
            # Pass clean_full_train_loss to log_metric if available (or 0.0)
            monitor.log_metric(epoch + 1, clean_full_train_loss if clean_full_train_loss else 0.0, avg_val_loss, val_acc, epoch_duration)
            ...
            if clean_full_train_loss is not None:
                log_dict['train/clean_full_train_loss'] = clean_full_train_loss
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ViT-small Training with DryShampoo Monitoring')
